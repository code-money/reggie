...
//        3、如果不需要处理，则直接放行
        ...
        if(request.getSession().getAttribute("employee")!=null){
            ...
            //获取当前登录用户
            Long sessionEmp =(Long) request.getSession().getAttribute("employee");
            //将当前登录的用户放到ThreadLocal中
            BaseContext.setCurrentId(sessionEmp);
            filterChain.doFilter(request,response);
            return;
        }
